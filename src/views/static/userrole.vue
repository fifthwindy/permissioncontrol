<template>
    <div class="head">
        ...
        <el-table :data="roles" style="width: 100%">
            ...
            <el-table-column prop="roleIds" label="角色编号" width="120" />
    ...
    <el-dialog v-model="EditDialogVisible" title="修改角色权限信息" width="50%" center>
        <el-form :model="role">
            ...
            <el-form-item label="角色编号" :label-width="formLabelWidth">
                <el-input v-model="role.roleIds" autocomplete="off" />
            </el-form-item>
            <el-form-item label="用户编号" :label-width="formLabelWidth">
                <el-input v-model="role.userId" autocomplete="off" />
            </el-form-item>
...
import { addRole, getRole, saveRole } from "@/http/index"//在http的文件夹里index.js 是相对应的
    ...
    methods: {
        ...
        getall() {
            ...
            getRole().then(res => {
                ...
                if (res.success) {
                    this.roles = res.data.roleIds.slice(start, end);//去浏览器的控制台看名字 我发的视频里也有
                    this.pageinfo.totalpage = res.data.roleIds.length;
        ...
        confirmAdd() {
            ...
            addRole(one).then(res => {//第116行 记得一起改 
        ...
        confirmEdit() {
            ...
            //console.log(dormitory)
            saveRole(role).then(res => {//这个函数在116行 改名记得一起
                if (res.success) {
